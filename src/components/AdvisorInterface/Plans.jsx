...
const Plans = () => {
  ...
  const handleInvest = async() => {
    ...
    const advisorData = JSON.parse(localStorage.getItem("advisorID"));
    let email = null;
    if(userData){
    ...
    try{
      const response = await fetch(`https://localhost:7244/api/requestPlan?userEmail=${email}`,
      {
        ...
        body : JSON.stringify({
          ...
          Risk: risk,
          AdvisorID: advisorData,
        })
      })

      if(!response.ok){
        throw new Error("Error updating plans!");
      }
  ...
  return (
    <div className="container-plans">
      ...
      <h3 className="heading-plans" style={{ textAlign: 'center',fontFamily:'Montserrat' }}>Create a Plan</h3>
